...
const CreatorCard = ({ ranking, userImage, username, totalSales}) => {
  return (
    <div className='flex flex-col justify-center items-center bg-background-secondary rounded-[20px] size-auto lg:size-60 relative'>
        ...
        <div className="flex flex-col justify-center items-center lg:items-start space-y-1 font-medium">
            <h3 className="text-h5 font-bold">@{username}</h3>
            <h4 className="text-sm opacity-80">Total Sales: <span className='text-sm font-normal'>{totalSales} ETH</span></h4>
        </div>
    </div>
  )
}
